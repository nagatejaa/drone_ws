...
inline bool land_drone(rclcpp::Node::SharedPtr node)
{
    ...
    if (!client->wait_for_service(std::chrono::seconds(5))) {
        RCLCPP_ERROR(node->get_logger(), "Service /mavros/cmd/land not available");
        return false;
    }
    ...
    if (rclcpp::spin_until_future_complete(node, result) == rclcpp::FutureReturnCode::SUCCESS && result.get()->success) {
        RCLCPP_INFO(node->get_logger(), "Landing command sent.");
        return true;
    }

    RCLCPP_ERROR(node->get_logger(), "Failed to send landing command");
    return false;
}
...
// Movement by distance and speed
inline void move_forward_by(rclcpp::Node::SharedPtr node, double meters, double speed)
{
    send_velocity(node, speed, 0.0, 0.0, meters / speed);
}

inline void move_backward_by(rclcpp::Node::SharedPtr node, double meters, double speed)
{
    send_velocity(node, -speed, 0.0, 0.0, meters / speed);
}

inline void move_right_by(rclcpp::Node::SharedPtr node, double meters, double speed)
{
    send_velocity(node, 0.0, -speed, 0.0, meters / speed);
}

inline void move_left_by(rclcpp::Node::SharedPtr node, double meters, double speed)
{
    send_velocity(node, 0.0, speed, 0.0, meters / speed);
}

inline void move_up_by(rclcpp::Node::SharedPtr node, double meters, double speed)
{
    send_velocity(node, 0.0, 0.0, speed, meters / speed);
}

inline void move_down_by(rclcpp::Node::SharedPtr node, double meters, double speed)
{
    send_velocity(node, 0.0, 0.0, -speed, meters / speed);
}
